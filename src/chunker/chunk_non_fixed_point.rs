...
pub fn chunk_q4_stable<T: BCAssigner>(
    constants: Vec<G2Prepared>,
    q4: ark_bn254::G2Affine,
    q4_input: G2PointType,
    assigner: &mut T,
    ...
    let num_constant = 3;
    // let line_coeffs = collect_line_coeffs(constants);
    let (line_coeffs,line_coeffs_tc) = collect_line_coeffs_stable(assigner, constants);
    let num_lines = line_coeffs.len();
    ...
    for i in (1..ark_bn254::Config::ATE_LOOP_COUNT.len()).rev() {
        for j in 0..num_line_groups {
            if j == num_constant {
                ...
                .add_parameter(&line_coeffs_tc[num_lines - (i + 2)][j][0].1)
                .add_parameter(&line_coeffs_tc[num_lines - (i + 2)][j][0].2)
                .add_parameter(&t4_acc)
                .add_result(&t4_update)
                .add_hint(hints);
            ...
            || ark_bn254::Config::ATE_LOOP_COUNT[i - 1] == -1
        {
            for j in 0..num_line_groups {
                if j == num_constant {
                    ...
                    t4_update.fill_with_data(crate::chunker::elements::DataType::G2PointData(t4x));
                    let segment = Segment::new_with_name(format!("check and add{}", i), script)
                        .add_parameter(&line_coeffs_tc[num_lines - (i + 2)][j][1].1)
                        .add_parameter(&line_coeffs_tc[num_lines - (i + 2)][j][1].2)
                        .add_parameter(&t4_acc)
                        .add_parameter(&q4_input)
                        .add_result(&t4_update)
                        .add_hint(hints);
    ...
    for j in 0..num_line_groups {
        if j == num_constant {
            ...
            t4_update.fill_with_data(crate::chunker::elements::DataType::G2PointData(t4x));
            let segment = Segment::new_with_name("final check and add".into(), script)
                .add_parameter(&line_coeffs_tc[num_lines - 2][j][0].1)
                .add_parameter(&line_coeffs_tc[num_lines - 2][j][0].2)                
                .add_parameter(&t4_acc)
                .add_parameter(&q4_input)
                .add_result(&t4_update)
                .add_hint(hints);
    ...
    for j in 0..num_line_groups {
        if j == num_constant {
            ...
            let segment = Segment::new_with_name("final_final check".into(), script)
                .add_parameter(&line_coeffs_tc[num_lines - 1][j][0].1)
                .add_parameter(&line_coeffs_tc[num_lines - 1][j][0].2)
                .add_parameter(&t4_acc)
                .add_parameter(&q4_input)
                .add_hint(hints);
            segments.push(segment);
...
mod tests {
    ...
    fn test_check_q4_stable() {
        ...
        for (_, segment) in segments.iter().enumerate() {
            ...
            let res = execute_script_with_inputs(script, witness);
            println!("segment {} exec_res {}", segment.name, res);
            let zero: Vec<u8> = vec![];
            assert_eq!(res.final_stack.len(), 1, "{}", segment.name); // only one element left
            assert_eq!(res.final_stack.get(0), zero, "{}", segment.name);
            assert!(
