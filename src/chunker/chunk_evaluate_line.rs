...
pub fn chunk_evaluate_line_stable_wrapper<T: BCAssigner>(
    assigner: &mut T,
    prefix: &str,
    f: ark_bn254::Fq12,
    x: ark_bn254::Fq,
    ...
    let mut t0 = Fq2Type::new(assigner, &format!("{}{}", prefix, "x"));
    t0.fill_with_data(Fq2Data(constant.0));
    let mut t1 = Fq2Type::new(assigner, &format!("{}{}", prefix, "x"));
    t1.fill_with_data(Fq2Data(constant.1));
    let mut t2 = Fq2Type::new(assigner, &format!("{}{}", prefix, "x"));
    t2.fill_with_data(Fq2Data(constant.2));
    ...
    let tc = (t0,t1,t2);
    chunk_evaluate_line_stable(assigner, prefix, pf, px, py, &tc, f, x, y, constant)
}


pub fn chunk_evaluate_line_stable<T: BCAssigner>(
    assigner: &mut T,
    prefix: &str,
    pf: Fq12Type,
    px: FqType,
...
#[cfg(test)]
mod test {
    use super::{chunk_evaluate_line_wrapper, chunk_evaluate_line_stable_wrapper};
    ...
    fn test_ell_stable() {
        ...
        let mut segments = Vec::new();
        let fn_name = format!("F_{}_mul_c_1p{}", 0, 0);
        let (segments_mul, mul): (Vec<segment::Segment>, elements::Fq12Type) = chunk_evaluate_line_stable_wrapper(
            &mut assigner,
            &fn_name,
            f,
            -p.x / p.y,
