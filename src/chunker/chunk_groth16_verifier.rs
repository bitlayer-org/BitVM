...
use crate::chunker::chunk_non_fixed_point::*;
...
fn groth16_verify_to_segments_stable<T: BCAssigner>(
    assigner: &mut T,
    public_inputs: &Vec<<Bn254 as ark_Pairing>::ScalarField>,
    proof: &Proof<Bn254>,
    vk: &VerifyingKey<Bn254>,
    ...
    let (segment, fs, f) =
        chunk_accumulator::chunk_accumulator_stable(assigner, tp_lst, q_prepared.to_vec(), c, c_inv, wi, p_lst);
    segments.extend(segment);
    ...
    let mut q4_input = G2PointType::new(assigner, "q4");
    q4_input.fill_with_data(crate::chunker::elements::DataType::G2PointData(q4));
    let segment = chunk_q4_stable(q_prepared.to_vec(), q4, q4_input, assigner);
...
mod tests {
    ...
    use crate::chunker::chunk_groth16_verifier::*;
    ...
    // fn test_hinted_groth16_verifier_stable_tool() -> (Vec<Sha256>, Vec<Segment>) {
        ...
        let mut assigner = DummyAssinger {};
        let segments = groth16_verify_to_segments_stable(&mut assigner, &vec![c], &proof, &vk);
