...
pub fn blake3_u32(num_u32: usize) -> Script {
    ...
    */
    ...
    // Add padding for u32
    let num_padding_u32s = num_padding_bytes / 4;
    ...
    let compression_script = script! {
        for i in 0..16 {
            {320 + 4*i} OP_ROLL // take one u32 input
            OP_DUP OP_TOALTSTACK // save a copy to altstack
            {limb_to_be_byte()} // decode to 4 byte
        }
        for _ in 0..320 {
            { 64 + 320 - 1 } OP_ROLL
        }
    ...
    let script = script! {
        ...
        // Add the padding
        { push_to_stack(0, num_padding_u32s) }

        // If padded, move all the bytes down
        if num_padding_u32s != 0 {
            for _ in 0..num_u32 {
                { num_u32 + num_padding_u32s - 1 } OP_ROLL
            }
        }
