...
pub fn fq2_sub() -> (ComputeFn, ScriptFn) {
    (
        ...
        Box::new(
            |compute_ctx: &ComputeCtx, inputs: Vec<State>| -> (Script, Vec<Vec<u8>>) {
                let script = crate::bn254::fq2::Fq2::sub(2, 0);
                (script, vec![])
            },
        ),
    )
...
pub fn fq2_div2() -> (ComputeFn, ScriptFn) {
    ...
    let script_fn = move |compute_ctx: &ComputeCtx, inputs: Vec<State>| -> (Script, Vec<Vec<u8>>) {
        assert!(inputs.len() == 1);
        let script = crate::bn254::fq2::Fq2::div2();
        (script, vec![])
    };
    (Box::new(func), Box::new(script_fn))
}
...
pub fn nonconstant_line_evaluate_c0(selector: Selector) -> (ComputeFn, ScriptFn) {
    ...
    let script_fn = move |compute_ctx: &ComputeCtx, inputs: Vec<State>| -> (Script, Vec<Vec<u8>>) {
        ...
        let (mul_c0_script, mul_c0_hint) =
            crate::bn254::fq::Fq::hinted_mul(3, lambda.c0, 0, p4.x().unwrap());
        let (mul_c1_script, mul_c1_hint) =
            crate::bn254::fq::Fq::hinted_mul(2, lambda.c1, 1, p4.x().unwrap());
        (
            script! {
                {crate::bn254::fq::Fq::drop()} // remove p4.y
                {crate::bn254::fq::Fq::copy(0)} // [c0, c1, p4x, p4x]
...
pub fn constant_line_eval_c1(
    ...
    let script_fn = move |compute_ctx: &ComputeCtx, inputs: Vec<State>| -> (Script, Vec<Vec<u8>>) {
        ...
        let v_neg = v.neg();
        let (mul_c0_script, mul_c0_hint) =
            crate::bn254::fq::Fq::hinted_mul(1, v_neg.c0, 2, p_point.y().unwrap());
        let (mul_c1_script, mul_c1_hint) =
            crate::bn254::fq::Fq::hinted_mul(1, v_neg.c1, 2, p_point.y().unwrap());

        let script = script! {
            {crate::bn254::fq::Fq::toaltstack()}
            {crate::bn254::fq::Fq::drop()} // remove p_point.x
            {crate::bn254::fq::Fq::fromaltstack()}
            {crate::bn254::fq::Fq::copy(0)} // [p_point.y, p_point.y]
            {crate::bn254::fq2::Fq2::push(v_neg)} // [p_point.y, p_point.y, c0, c1]
            {mul_c0_script} // [p_point.y, c1, c0']
            {mul_c1_script} // [c0', c1']
        };
