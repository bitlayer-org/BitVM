...
pub fn generate_mock_segments(vk: &ark_groth16::VerifyingKey<Bn254>) -> Vec<Segment> {
    println!("generate_partial_script");
    assert!(vk.gamma_abc_g1.len() == NUM_PUBS + 1);
...
pub fn partial_scripts_from_segments(segments: &Vec<Segment>) -> Vec<treepp::Script> {
